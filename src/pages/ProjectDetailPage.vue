    ...
    <div class="py-16">
        <div class="container mx-auto px-4">
            ...
            <div v-else-if="error"
                ...
                <h2 class="text-2xl font-semibold text-red-800 dark:text-red-200 mb-2">Proje Bulunamadı</h2>
                <p class="text-red-700 dark:text-red-300 mb-6">İstediğiniz proje bulunamadı veya bir hata oluştu.</p>
                <router-link to="/projects"
                    class="inline-block px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors">Projelere
                    Dön</router-link>
            </div>
            ...
            <div v-else-if="project" class="max-w-7xl mx-auto">
                ...
                <div class="flex flex-col md:flex-row items-start mb-10 gap-8">
                    ...
                    <div class="w-full md:w-3/5">
                        <h1 class="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">{{ project.title
                            }}</h1>
                        ...
                        <div v-if="project.short_description"
                            ...
                            <p class="text-blue-800 dark:text-blue-200">{{ project.short_description }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                                ...
                                <div v-if="!project.project_url && !project.github_url"
                                    ...
                                    Proje linkleri bulunmamaktadır</div>
                            </div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                                ...
                                <div v-else class="text-gray-500 dark:text-gray-400 text-sm">Teknoloji bilgisi
                                    girilmemiş</div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="project.description" class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 mb-10">
                    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Proje Açıklaması</h2>
                    <div class="prose prose-lg dark:prose-invert max-w-none">{{ project.description }}</div>
                </div>

                <!-- Diğer Projeler Alanı -->
                <div class="mt-16">
                    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-6">Diğer Projeler</h2>
                            ...
                            <div
                                ...
                                <template v-else>
                                    <div class="text-gray-400 dark:text-gray-500">
                                        ...
                                        <p class="mt-2">Görsel bulunmamaktadır</p>
                                    </div>
                                </template>
                            </div>
                            <div class="p-4">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{
                                    relatedProject.title }}</h3>
                                <p class="text-gray-600 dark:text-gray-400 text-sm line-clamp-2">{{
                                    relatedProject.short_description }}
                                </p>
                                <router-link :to="`/projects/${relatedProject.id}`"
                                    class="mt-3 inline-block text-blue-600 dark:text-blue-400 hover:underline text-sm">Detayları
                                    Gör</router-link>
                            </div>
                        </div>
                    </div>
...
const project = computed(() => {
    return projectsStore.currentProject || null;
});

// Teknoloji yığını formatlama
const formattedTechStack = computed(() => {
    const techStack = project.value?.tech_stack;
...
// Sayfa yüklendiğinde ve route id değiştiğinde çalıştır
onMounted(() => {
    loadProject();
});
