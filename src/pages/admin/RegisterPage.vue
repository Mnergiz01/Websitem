      ...
      <div class="max-w-md w-full space-y-8 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
        ...
        <div class="text-center">
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Portfolyo admin paneline erişim için kayıt olun
          </p>
        </div>
        ...
        <form class="mt-8 space-y-6" @submit.prevent="handleRegister">
          ...
          <div class="text-center mt-4">
            <p class="text-sm text-gray-600 dark:text-gray-400">
              ...
              <router-link :to="{ name: 'Login' }" class="font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400 dark:hover:text-blue-300">
                Giriş Yap
              </router-link>
            </p>
          </div>
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (result && result.user) {
        ...
        // Some providers may not return the user directly, handle this case
        successMessage.value = 'Kayıt başarılı! Giriş yapmak için email adresinizi kontrol edin.';
      }
    } catch (error) {
      console.error('Registration failed:', error);
      errorMessage.value = error.message || 'Kayıt başarısız. Lütfen tekrar deneyin.';
    } finally {
